#!/usr/bin/env python3
# notepadpp_full.py
# A monolithic, feature-rich Notepad++-like editor in Python using PyQt6 + QScintilla
# Includes: tabs, folding, lexers, bookmarks, search/replace, find-in-files (dock),
# encoding & EOL handling, recent files, session restore (tabs + caret), split view,
# function list (dock), macro recording/playback (basic), plugins loader stubs,
# simple external tools runner, drag & drop, toolbar + status bar polish, themes.
#
# Requires:
#   pip install PyQt6 PyQt6-QScintilla
#
# Run:
#   python notepadpp_full.py
...
from PyQt6.QtGui import (
    QAction, QIcon, QKeySequence, QCloseEvent, QGuiApplication, QPalette
)
from PyQt6.QtWidgets import (
    ...
    QHBoxLayout, QListWidget, QListWidgetItem, QMenu, QSplitter, QTextEdit
)
...
APP_ORG = "OpenDev"
APP_NAME = "NotepadPP-Full"
MAX_RECENTS = 15
SESSION_FILE = str(Path.home() / ".npp_full_session.json")
...
class SciEditor(QsciScintilla):
    ...
    def __init__(self, parent=None):
        ...
        self._macro: List[Tuple[str, str]] = []  # (type, data), e.g., ("text", "a"), ("key","Backspace")
        self._init_editor()
    ...
    # -------- Load/Save
    def load_from_file(self, path: str, encoding_choices: List[str] = ["utf-8", "latin-1"]) -> bool:
        err = None
        for enc in encoding_choices:
            try:
                ...
                self.file_state.encoding = enc
                try:
                    self.file_state.mtime = os.path.getmtime(path)
                except Exception:
                    self.file_state.mtime = None
    ...
    def wheelEvent(self, e):
        if e.modifiers() & Qt.KeyboardModifier.ControlModifier:
            ...
            if delta > 0:
                ...
                super().zoomOut()
                self._zoom_steps -= 1
            self.zoomChangedX.emit(self.zoom_percent())
            e.accept()
            return
        super().wheelEvent(e)
    ...
    def next_bookmark_line(self, from_line: int) -> Optional[int]:
        # brute force scan for marker presence
        maxl = self.lines()
        for l in range(from_line + 1, maxl):
            if self.markersAtLine(l) & (1 << self.MARK_BOOKMARK):
                return l
    ...
    def event(self, e):
        if self._recording and e.type() == QEvent.Type.KeyPress:
            ...
            text = k.text()
            if text:
                self._macro.append(("text", text))
            else:
                key = k.key()
                name = {Qt.Key.Key_Backspace:"Backspace", Qt.Key.Key_Tab:"Tab", Qt.Key.Key_Return:"Return", Qt.Key.Key_Enter:"Return"}.get(key)
                if name:
                    self._macro.append(("key", name))
...
class FindReplaceDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        row = QHBoxLayout()
        for w in (self.case_cb, self.word_cb, self.regex_cb, self.sel_cb, self.inc_cb):
            row.addWidget(w)
        wrap = QWidget(); wrap.setLayout(row)
        grid.addWidget(wrap, 2, 0, 1, 5)

        row2 = QHBoxLayout()
        for b in (btn_find, btn_prev, btn_repl, btn_all, btn_close):
            row2.addWidget(b)
        wrap2 = QWidget(); wrap2.setLayout(row2)
        grid.addWidget(wrap2, 3, 0, 1, 5)
...
class FindInFilesDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        row = QHBoxLayout()
        for w in (self.case_cb, self.word_cb, self.regex_cb, self.recur_cb):
            row.addWidget(w)
    ...
    def _run(self):
        ...
        masks = self.masks_edit.text().strip()
        text = self.text_edit.text()
        if not root or not text:
            QMessageBox.warning(self, "Find in Files", "Choose folder and enter text to search.")
            return
        # Mask string passed, recursion handled by main window
...
class ResultsDock(QDockWidget):
    def __init__(self, parent=None):
        ...
        w = QWidget(); lay = QVBoxLayout(w); lay.setContentsMargins(0,0,0,0); lay.addWidget(self.tree)
        self.setWidget(w)
...
class FunctionListDock(QDockWidget):
    def __init__(self, parent=None):
        ...
        self.setObjectName("FunctionListDock")
        self.list = QListWidget()
        w = QWidget(); lay = QVBoxLayout(w); lay.setContentsMargins(0,0,0,0); lay.addWidget(self.list)
        self.setWidget(w)
...
class ConsoleDock(QDockWidget):
    def __init__(self, parent=None):
        ...
        w = QWidget(); lay = QVBoxLayout(w); lay.setContentsMargins(0,0,0,0); lay.addWidget(self.out)
        self.setWidget(w)
...
class EditorTab(QWidget):
    def __init__(self, path: Optional[str] = None, parent=None):
        super().__init__(parent)
        self.editor = SciEditor()
        self.editor.apply_lexer_for(path)
        lay = QVBoxLayout(self)
        lay.setContentsMargins(0, 0, 0, 0)
        lay.addWidget(self.editor)
        if path:
            self.editor.load_from_file(path)
...
class MainWindow(QMainWindow):
    ...
    def _current_tabs(self) -> QTabWidget:
        if self.tabs_right.isVisible() and self.tabs_right.hasFocus():
    ...
    def new_file(self, right: bool=False):
        tab = EditorTab(None)
        ed = tab.editor
        ed.caretMovedX.connect(self._update_status_pos)
        ed.zoomChangedX.connect(lambda pct: self.zoom_label.setText(f"{pct}%"))
    ...
    def _open_path(self, path: str, in_other: bool=False):
        ...
        tab = EditorTab(path)
        ed = tab.editor; ed.caretMovedX.connect(self._update_status_pos); ed.zoomChangedX.connect(lambda pct: self.zoom_label.setText(f"{pct}%"))
    ...
    def save_file(self, save_as: bool=False):
        ...
        if save_as or not path:
            ...
            if ed.save_to_file(new_path):
                self._set_cur_tab_title(os.path.basename(new_path))
                self._add_recent(new_path)
        else:
            if ed.save_to_file(path):
    ...
    def closeEvent(self, e: QCloseEvent):
        # TODO prompt unsaved tabs (simplified: no prompt per instruction brevity)
        self._save_session()
        super().closeEvent(e)

    def _close_tab(self, tabs: QTabWidget, index: int):
        tabs.removeTab(index)
        if tabs.count() == 0 and tabs is self.tabs_left:
            self.new_file()
    ...
    def _rebuild_recents_menu(self):
        ...
        rec = self.settings.value("recent_files", [], list)
        if not rec:
            a = self.recent_menu.addAction("(Empty)"); a.setEnabled(False)
        else:
            for p in rec:
                act = self.recent_menu.addAction(p)
                act.triggered.connect(lambda _, path=p: self._open_path(path))
            self.recent_menu.addSeparator()
            self.recent_menu.addAction("Clear List", lambda: (self.settings.setValue("recent_files", []), self._rebuild_recents_menu()))

    # ---------- Search ----------
    def _qflags(self, case: bool, whole: bool, regex: bool):
        flags = 0
        if not regex:
            # we handle with Python regex; flags used only for state
            pass
        return flags
    ...
    def _find_in_text(self, ed: SciEditor, pattern: str, cs: bool, ww: bool, rx: bool, sel_only: bool, backwards: bool) -> bool:
        ...
        if sel_only and ed.hasSelectedText():
            s = ed.selectedText()
            # Qsci returns selection with newlines as \r\n
            text = s.replace('\r\n', '\n')
            # use start pos as selection start
        ...
        if backwards:
            m = None
            for mm in R.finditer(text, 0, start_pos):
                m = mm
        else:
        ...
        # map to positions in full text
        s, e = m.start(), m.end()
        l1, c1 = self._linecol_from_pos(ed.text(), s)
        l2, c2 = self._linecol_from_pos(ed.text(), e)
        ed.setSelection(l1, c1, l2, c2)
        ed.ensureLineVisible(l1)
        return True
    ...
    def _replace_all(self, pat, rep, cs, ww, rx, sel_only):
        ...
        if sel_only and ed.hasSelectedText():
            l1, c1, l2, c2 = ed.getSelection()
            sel_text = ed.selectedText().replace('\r\n', '\n')
            R = re.compile(pat if rx else re.escape(pat), 0 if cs else re.IGNORECASE | re.MULTILINE)
            new_sel = R.sub(rep, sel_text)
            ed.setSelection(l1, c1, l2, c2); ed.replaceSelectedText(new_sel)
        else:
            R = re.compile(pat if rx else re.escape(pat), 0 if cs else re.IGNORECASE | re.MULTILINE)
            new = R.sub(rep, text)
            ed.setText(new)
    ...
    def _find_next(self, pat, cs, ww, rx, sel_only, backwards):
        ...
        ok = self._find_in_text(ed, pat, cs, ww, rx, sel_only, backwards)
        if not ok:
            self.status.showMessage("No matches", 1500)
    ...
    def _run_find_in_files(self, root: str, masks: str, text: str, case: bool, whole: bool, regex: bool):
        ...
        try:
            ...
            QMessageBox.warning(self, "Regex error", str(e)); return
        ...
        for f in iter_files():
            ...
            for m in R.finditer(data):
                ...
                item = QTreeWidgetItem([str(f), str(line), str(col+1), snippet])
                self.results_dock.tree.addTopLevelItem(item)

    def _open_result_item(self, item: QTreeWidgetItem, _col: int):
        path = item.text(0); line = int(item.text(1)); col = int(item.text(2))
        self._open_path(path)
        ed = self._editor()
        if not ed: return
        ed.setCursorPosition(line-1, col-1); ed.ensureLineVisible(line-1)
        ed.setSelection(line-1, col-1, line-1, col)

    # ---------- Function list ----------
    def _update_function_list(self):
        ed = self._editor()
        if not ed: return
        path = ed.file_state.path or ""
        text = ed.text()
    ...
    def _update_status_all(self):
        ...
        self.eol_label.setText(ed.eol_str())
        self.enc_label.setText(ed.file_state.encoding.upper())
        self.zoom_label.setText(f"{ed.zoom_percent()}%")
        self.mod_label.setText("MOD" if ed.isModified() else "")
    ...
    def _make_actions(self):
        # File
        self.act_new = QAction(QIcon.fromTheme("document-new"), "&New", self, shortcut=QKeySequence.StandardKey.New, triggered=self.new_file)
        self.act_open = QAction(QIcon.fromTheme("document-open"), "&Open…", self, shortcut=QKeySequence.StandardKey.Open, triggered=self.open_files)
        self.act_reload = QAction("Re&load from Disk", self, shortcut="Ctrl+R", triggered=self.reload_from_disk)
        self.act_save = QAction(QIcon.fromTheme("document-save"), "&Save", self, shortcut=QKeySequence.StandardKey.Save, triggered=self.save_file)
        self.act_save_as = QAction(QIcon.fromTheme("document-save-as"), "Save &As…", self, shortcut="Ctrl+Shift+S", triggered=lambda: self.save_file(True))
        ...
        # Edit
        self.act_undo = QAction(QIcon.fromTheme("edit-undo"), "&Undo", self, shortcut=QKeySequence.StandardKey.Undo, triggered=lambda: self._editor() and self._editor().undo())
        self.act_redo = QAction(QIcon.fromTheme("edit-redo"), "&Redo", self, shortcut=QKeySequence.StandardKey.Redo, triggered=lambda: self._editor() and self._editor().redo())
        self.act_cut = QAction(QIcon.fromTheme("edit-cut"), "Cu&t", self, shortcut=QKeySequence.StandardKey.Cut, triggered=lambda: self._editor() and self._editor().cut())
        self.act_copy = QAction(QIcon.fromTheme("edit-copy"), "&Copy", self, shortcut=QKeySequence.StandardKey.Copy, triggered=lambda: self._editor() and self._editor().copy())
        self.act_paste = QAction(QIcon.fromTheme("edit-paste"), "&Paste", self, shortcut=QKeySequence.StandardKey.Paste, triggered=lambda: self._editor() and self._editor().paste())
        ...
        # Search
        self.act_find = QAction(QIcon.fromTheme("edit-find"), "&Find/Replace…", self, shortcut=QKeySequence.StandardKey.Find, triggered=lambda: (self.find_dialog.show(), self.find_dialog.raise_()))
        ...
        self.act_next_bookmark = QAction("Next &Bookmark", self, shortcut="F2", triggered=lambda: self._jump_bookmark(next=True))
        self.act_prev_bookmark = QAction("&Previous Bookmark", self, shortcut="Shift+F2", triggered=lambda: self._jump_bookmark(next=False))
        ...
        # Tools
        self.act_run_python = QAction("Run &Python (current file)", self, triggered=self._run_python_current)
    ...
    def _make_menus(self):
        ...
        m_file.addSeparator()
        self.recent_menu = m_file.addMenu("Open &Recent"); self._rebuild_recents_menu()
        m_file.addSeparator()
        ...
        self._load_plugins()  # initial

        # Tools
        m_tools = mb.addMenu("&Tools")
        for a in (self.act_run_python, self.act_external_cmd): m_tools.addAction(a)

        # Help
        m_help = mb.addMenu("&Help")
        m_help.addAction("About", lambda: QMessageBox.information(self, "About", f"<b>{APP_NAME}</b><br>Notepad++-like editor in Python.<br>© 2025 MIT"))
    ...
    def _toggle_wrap(self, on: bool):
        ed = self._editor()
        if ed:
            ed.setWrapMode(QsciScintilla.WrapMode.WrapWord if self.act_wrap.isChecked() else QsciScintilla.WrapMode.WrapNone)

    def _toggle_ws(self, on: bool):
        ed = self._editor()
        if ed:
            ed.setWhitespaceVisibility(QsciScintilla.WhitespaceVisibility.WsVisible if self.act_ws.isChecked() else QsciScintilla.WhitespaceVisibility.WsInvisible)

    def _toggle_eol(self, on: bool):
        ed = self._editor()
        if ed:
            ed.setEolVisibility(self.act_eol.isChecked())
    ...
    def _reopen_encoding(self, enc: str):
        ed = self._editor()
        if not ed or not ed.file_state.path: return
        ed.load_from_file(ed.file_state.path, [enc])
        self.enc_label.setText(ed.file_state.encoding.upper())
    ...
    def _convert_encoding(self, enc: str, lossy: bool=False):
        ...
        try:
            ...
            ed.setText(data)
            ed.file_state.encoding = enc
            self.enc_label.setText(enc.upper())
        except Exception as e:
    ...
    def _jump_bookmark(self, next: bool=True):
        ...
        if target is not None:
            ed.setCursorPosition(target, 0); ed.ensureLineVisible(target)

    # ---------- Plugins ----------
    def _load_plugins(self):
        # Create plugin dir if missing
        Path(PLUGINS_DIR).mkdir(parents=True, exist_ok=True)
        # Clear previous plugin items (keep reload/open-dir actions at top)
        # Add a separator after static items
        while self.m_plugins.actions() and self.m_plugins.actions()[-1].text() != "":
            pass
        # Remove dynamic part: everything after the second separator
        while self.m_plugins.actions() and self.m_plugins.actions()[-1].isSeparator():
            self.m_plugins.removeAction(self.m_plugins.actions()[-1])
        # Add separator
        self.m_plugins.addSeparator()
        # Load *.py in plugins dir
            ...
            try:
                ...
                if hasattr(mod, "register"):
                    ...
                    try:
                        ...
                        submenu.addAction(f"Error: {e}")
                else:
                    self.m_plugins.addAction(f"{pyf.stem} (no register())").setEnabled(False)
            except Exception as e:
                self.m_plugins.addAction(f"{pyf.stem} (load error)").setEnabled(False)

    # ---------- Tools ----------
    def _run_python_current(self):
        ed = self._editor()
        if not ed or not ed.file_state.path:
            QMessageBox.information(self, "Run Python", "Save the file first.")
            return
    ...
    def _run_external(self):
        from PyQt6.QtWidgets import QInputDialog
        ed = self._editor()
        cur_file = ed.file_state.path if ed and ed.file_state.path else ""
        cmd_tpl, ok = QInputDialog.getText(self, "Run External", "Command (use {file} placeholder):", text="echo Running {file}")
        if not ok: return
        cmd = cmd_tpl.replace("{file}", shlex.quote(cur_file))
        self._run_command(cmd, cwd=os.path.dirname(cur_file) if cur_file else None)

    def _run_command(self, cmd: str, cwd: Optional[str]=None):
        self.console_dock.show(); self.console_dock.raise_()
        self.console_dock.out.append(f"$ {cmd}\n")
        try:
    ...
    def _tab_context_menu(self, tabs: QTabWidget, pos: QPoint):
        ...
        if act == act_close:
            ...
            if path:
                ...
                self._open_path(path, in_other=True)
                self.tabs_right.setVisible(True)
    ...
    def _save_session(self):
        ...
        try:
            Path(SESSION_FILE).write_text(json.dumps(sess, indent=2))
        except Exception:
            pass
        ...
        for label, tabs in (("left", self.tabs_left), ("right", self.tabs_right)):
            for ent in data.get(label, []):
                ...
                if path and os.path.exists(path):
                    ...
                    ed = self._editor()
                    if ed:
                        ed.set_eol(ent.get("eol","LF"))
                        ed.file_state.encoding = ent.get("encoding","utf-8")
