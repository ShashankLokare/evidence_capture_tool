#!/usr/bin/env python3
# notepadpp_full_pro.py
# A premium macOS-presentable Notepad++-style editor in one file.
# Tech: PyQt6 + QScintilla + SpeechRecognition + sounddevice (no PyAudio required)
#
# Big feature set:
# - Premium dark QSS across the app (tabs, toolbar, menus, docks, status bar)
# - Auto monospace font: SF Mono → Menlo → Monaco → Courier New → system Fixed
# - QScintilla editor: line numbers, folding, indentation guides, brace match
# - Syntax highlight: Python/C/C++/Java/JS/HTML/JSON/Bash/Fortran
# - Tabs (closable, draggable), Split View (left/right tab bars), clone tab
# - Search: Find/Replace (regex/whole/case/selection/incremental)
# - Find in Files dock (masks, recursive), double-click to jump
# - Function List dock (heuristics for Python/C-like/Fortran), jump to symbol
# - Bookmarks (gutter), next/prev (F2 / Shift+F2)
# - Go To Line; Quick Find box in toolbar
# - Encoding & EOL: reopen/convert UTF-8/Latin-1; LF/CRLF/CR conversion; show EOL/WS
# - Status bar: Ln/Col, EOL, Encoding, Zoom%, MOD flag
# - Session restore (tabs, views, cursor pos, encoding/EOL); auto-reload on disk change
# - Drag & drop to open; Recent files menu
# - Macros: record / stop / play (basic keystrokes)
# - Plugins: load plugins/*.py with register(app, menu)
# - Tools: Run Python (current), Run External ({file}); Dictation with sounddevice
# - Self Test Report
#
# Install:
#   pip install PyQt6 PyQt6-QScintilla SpeechRecognition sounddevice numpy
# Run:
#   python notepadpp_full_pro.py

from __future__ import annotations
import os, sys, re, json, fnmatch, importlib.util, subprocess, shlex, time
from dataclasses import dataclass
from typing import Optional, List, Tuple
from pathlib import Path

from PyQt6.QtCore import Qt, QSettings, QSize, QTimer, QEvent, pyqtSignal, QPoint
from PyQt6.QtGui import (
    QAction, QKeySequence, QCloseEvent, QFont, QFontDatabase, QColor, QIcon, QGuiApplication
)
from PyQt6.QtWidgets import (
    ...
    QHBoxLayout, QListWidget, QListWidgetItem, QMenu, QSplitter, QTextEdit, QStyle, QInputDialog
)
...
def best_mono_font(size: int = 13) -> QFont:
    # Use static QFontDatabase methods (no instantiation — avoids TypeError)
...
DARK = {
    ...
    "func": QColor("#DCDCAA"),
    "class": QColor("#4EC9B0"),
    "accent": QColor("#0a84ff"),  # macOS blue
...
class MainWindow(QMainWindow):
    ...
    # Dictation (sounddevice primary, PyAudio/Microphone fallback if available)
    def _start_dictation(self):
        ...
        # Try sounddevice first (no PyAudio build needed)
        try:
            import sounddevice as sd
            import numpy as np
            self.status.showMessage("🎙️ Listening (sounddevice)… speak for ~7 seconds", 4000)
            ...
            # Fallback to Microphone (requires PyAudio)
            try:
                self.status.showMessage("🎙️ Listening (PyAudio)…", 3000)
                ...
                QMessageBox.warning(self, "Dictation error", f"{e}\n\nFallback also failed:\n{e2}\n\nTip: pip install sounddevice numpy")
...
def main():
    ...
    # Set base UI font for non-editor widgets
